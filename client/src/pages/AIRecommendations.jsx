    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="bg-gradient-to-r from-slate-700 via-slate-600 to-slate-500 rounded-xl p-6 text-white shadow-xl">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button 
            onClick={handleScan}
            disabled={loading}
            className="bg-white dark:bg-gray-800 text-slate-700 dark:text-gray-300 px-6 py-3 rounded-lg font-semibold hover:bg-slate-50 dark:hover:bg-gray-700 transition-all flex items-center gap-2 disabled:opacity-50 shadow-lg"
          >
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border-2 border-slate-200 dark:border-gray-700 shadow-md">
            <p className="text-sm text-slate-600 dark:text-gray-400 mb-1 font-medium">Total Opportunities</p>
            <p className="text-xl sm:text-3xl font-bold text-slate-900 dark:text-gray-100">{summary.total}</p>
          </div>
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-lg font-semibold whitespace-nowrap transition-all shadow-sm ${
              activeFilter === filter 
                ...
                : 'bg-white dark:bg-gray-800 text-slate-700 dark:text-gray-300 border-2 border-slate-200 dark:border-gray-700 hover:border-slate-300 dark:hover:border-gray-600 hover:bg-slate-50 dark:hover:bg-gray-700'
            }`}
          >
      ...
      <div className="space-y-5">
        {filteredStocks.map((stock, index) => {
          ...
          return (
            <div 
              key={index} 
              className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg border-2 ${getRiskBorder(stock.riskCategory)} overflow-hidden hover:shadow-xl transition-all`}
            >
              {/* Premium Header */}
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    <div className={`bg-gradient-to-br ${getRiskBg(stock.riskCategory)} rounded-xl p-4 border ${getRiskBorder(stock.riskCategory)}`}>
                      <p className="text-sm text-slate-600 dark:text-gray-400 mb-1 font-medium">Suggested Investment</p>
                      <p className="text-2xl font-bold text-slate-900 dark:text-gray-100">₹{stock.suggestedAmount?.toLocaleString('en-IN')}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-3 border-2 border-emerald-200">
                        <p className="text-xs text-slate-600 dark:text-gray-400 font-medium">🎯 Target</p>
                      ...
                      <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-lg p-3 border-2 border-red-200">
                        <p className="text-xs text-slate-600 dark:text-gray-400 font-medium">🛑 Stop Loss</p>
                  ...
                  {/* Simple Explanation */}
                  <div>
                    <h4 className="text-lg font-bold text-slate-900 dark:text-gray-100 mb-3 flex items-center gap-2">
                      <Lightbulb className="w-5 h-5 text-amber-500" />
                      Why This Stock?
                    </h4>
                    <ul className="space-y-2">
                      {stock.simpleWhy?.map((reason, i) => (
                        <li key={i} className="flex items-start gap-2 text-slate-700 dark:text-gray-300">
                ...
                {/* Expected Returns */}
                <div className="mt-5 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-800 rounded-xl p-4 border-2 border-slate-200 dark:border-gray-700">
                  <h4 className="text-sm font-bold text-slate-700 dark:text-gray-300 mb-3">Expected Returns</h4>
                  <div className="grid grid-cols-3 gap-3 text-center text-sm">
                    <div>
                      <p className="text-slate-600 dark:text-gray-400 mb-1">🚀 Best Case</p>
                      <p className="text-lg font-bold text-emerald-600">{stock.expectedReturns?.best}</p>
                    </div>
                    <div>
                      <p className="text-slate-600 dark:text-gray-400 mb-1">📊 Likely</p>
                      <p className="text-lg font-bold text-slate-700 dark:text-gray-300">{stock.expectedReturns?.likely}</p>
                    </div>
                    <div>
                      <p className="text-slate-600 dark:text-gray-400 mb-1">📉 Worst</p>
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button className="bg-slate-100 dark:bg-gray-700 hover:bg-slate-200 dark:hover:bg-gray-600 text-slate-700 dark:text-gray-300 font-semibold py-3 px-6 rounded-lg transition-all border-2 border-slate-200 dark:border-gray-700">
                    ⭐ Watchlist
                  </button>
                </div>
              </div>
      ...
      {filteredStocks.length === 0 && !loading && (
        <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-gray-900 dark:to-gray-800 rounded-xl p-12 text-center border-2 border-slate-200 dark:border-gray-700 shadow-lg">
          <Lightbulb className="w-16 h-16 text-slate-300 dark:text-gray-600 mx-auto mb-4" />
          {scanError ? (
            <>
            ...
            <>
              <p className="text-slate-700 dark:text-gray-300 text-lg font-semibold mb-2">No opportunities yet</p>
              <p className="text-slate-500 dark:text-gray-400 text-sm mb-5">Run a deep market scan to find investment opportunities</p>
            </>
          )}
          <button
            onClick={handleScan}
      ...
      <div className="bg-amber-50 dark:bg-yellow-900/30 border-2 border-amber-200 dark:border-amber-700 rounded-xl p-4 text-sm">
